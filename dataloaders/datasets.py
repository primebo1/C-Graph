...
class TestDataset(Dataset):
    ...
    def __init__(self, args):
        ...
        # reading the paths
        if args['dataset'] == 'CARDIAC_bssFP':
            self.image_dirs = glob.glob(os.path.join(args['data_dir'], 'cmr_bssFP_normalized/image*'))
        elif args['dataset'] == 'CARDIAC_LGE':
            self.image_dirs = glob.glob(os.path.join(args['data_dir'], 'cmr_LGE_normalized/image*'))
        elif args['dataset'] == 'ABDOMEN_MR':
...
class TrainDataset(Dataset):
    ...
    def __init__(self, args):
        ...
        if args['dataset'] == 'CARDIAC_bssFP':
            self.image_dirs = glob.glob(os.path.join(args['data_dir'], 'cmr_bssFP_normalized/image*'))
            self.label_dirs = glob.glob(os.path.join(args['data_dir'], 'cmr_bssFP_normalized/label*'))
        elif args['dataset'] == 'CARDIAC_LGE':
            self.image_dirs = glob.glob(os.path.join(args['data_dir'], 'cmr_LGE_normalized/image*'))
            self.label_dirs = glob.glob(os.path.join(args['data_dir'], 'cmr_LGE_normalized/label*'))
        elif args['dataset'] == 'ABDOMEN_MR':
            self.image_dirs = glob.glob(os.path.join(args['data_dir'], 'chaos_MR_T2_normalized/image*'))
    ...
    def geom_transform(self, img, mask, supervoxel):
        ...
        if len(img.shape) > 4:
            ...
            for shot in range(n_shot):
                # 处理 img, mask 和 supervoxel
                cat_img_mask = np.concatenate((img[0, shot], mask[:, shot])).transpose(1, 2, 0)
            ...
            for q in range(img.shape[0]):
                # 处理 img, mask 和 supervoxel
                cat_img_mask = np.concatenate((img[q], mask[q][None])).transpose(1, 2, 0)
                # print(cat_img_mask.dtype)
                # print(supervoxel[q].dtype)
                cat_supervoxel = supervoxel[q].transpose(1, 2, 0)
                
                cat_img_mask = transform(cat_img_mask).transpose(2, 0, 1)
                cat_supervoxel = transform(cat_supervoxel).transpose(2, 0, 1)
                
                img[q] = cat_img_mask[:3, :, :]
                mask[q] = np.rint(cat_img_mask[3:, :, :].squeeze())
                supervoxel[q] = np.rint(cat_supervoxel)
    ...
    def __getitem__(self, idx):
        ...
        whole_supervoxel = whole_supervoxel[sample[:self.n_shot * self.n_way]][None,]
        ...
        # geom transform
        if np.random.random(1) > 0.5:
            qry_img, qry_lbl, _ = self.geom_transform(qry_img, qry_lbl, whole_supervoxel.astype(np.float64))   
        else:
            sup_img, sup_lbl, superv = self.geom_transform(sup_img, sup_lbl, whole_supervoxel.astype(np.float64))
