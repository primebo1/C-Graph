...
# from models.bilateral_cdfs import FewShotSeg
...
def main(_run, _config, _log):
            ...
            for i, sample in enumerate(test_loader):  # this "for" loops 4 times
                ...
                for sub_chunck in range(_config['n_part']):  # n_part = 3
                    ...
                    for i in range(query_image_s.shape[0]):
                        _pred_s, _ = model([support_image_s], [support_fg_mask_s], [query_image_s[[i]]], query_label_s ,
    ...
    # 将每个类别的 Dice 和 IoU 保存到文件 
    # with open('class_dice.txt', 'a') as file: 
    #     file.write("Per-class Dice and IoU:\n")
    #     for label, dice_score in class_dice.items():
    #         file.write(f"Class {label}: Dice = {dice_score}, IoU = {class_iou[label]}\n")
        
    #     file.write("\nOverall Mean:\n")
    #     file.write(f"Mean Dice: {dict_Avg(class_dice)}\n")
    #     file.write(f"Mean IoU: {dict_Avg(class_iou)}\n")
