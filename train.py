...
def main(_run, _config, _log):
    ...
    log_loss = {'total_loss': 0, 'query_loss': 0, 'contrast_loss': 0}
    ...
    for sub_epoch in range(n_sub_epochs):
        ...
        for _, sample in enumerate(train_loader):
            ...
            whole_supervoxel = sample['whole_supervoxel'].long().cuda()[0][0][0] # 256,256
        
            # Compute outputs and losses.
            query_pred, cnc_loss = model(support_images, support_fg_mask, query_images, query_labels ,train=True)
            ...
            loss = query_loss + cnc_loss 

            if loss.isnan():
                _log.info('NaN loss encountered. Skipping...')
                # save a empty file to indicate the error
                with open(f'./error.txt', 'w') as f:
                    f.write('NaN loss encountered. Skipping...')
                return 0
